import events from 'events'
import humanizeDuration from 'humanize-duration'

/**
 * Initialize a new `spec` test reporter.
 *
 * @param {Runner} runner
 * @api public
 */
class SpecReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        super()

        this.baseReporter = baseReporter
        this.config = config
        this.options = options
        this.durationOpts = { units: ['m', 's'], round: true, spacer: '' }
        this.shortEnglishHumanizer = humanizeDuration.humanizer({
            language: 'shortEn',
            languages: { shortEn: {
                h: () => 'h',
                m: () => 'm',
                s: () => 's',
                ms: () => 'ms'
            }}
        })

        const { epilogue } = this.baseReporter

        this.indents = {}
        this.suiteIndents = {}
        this.specs = {}
        this.results = {}

        this.on('runner:start', function (runner) {
            this.suiteIndents[runner.cid] = {}
            this.indents[runner.cid] = 0
            this.specs[runner.cid] = runner.specs
            this.results[runner.cid] = {
                passing: 0,
                pending: 0,
                failing: 0
            }
        })

        this.on('suite:start', function (suite) {
            this.suiteIndents[suite.cid][suite.title] = ++this.indents[suite.cid]
        })

        this.on('test:pending', function (test) {
            this.results[test.cid].pending++
        })

        this.on('test:pass', function (test) {
            this.results[test.cid].passing++
        })

        this.on('test:fail', function (test) {
            this.results[test.cid].failing++
        })

        this.on('suite:end', function (suite) {
            this.indents[suite.cid]--
        })

        this.on('runner:end', function (runner) {
            this.printSuiteResult(runner)
        })

        this.on('end', function () {
            epilogue.call(baseReporter)
            console.log()
        })
    }

    indent (cid, specTitle) {
        const indents = this.suiteIndents[cid][specTitle]
        return indents < 0 ? '' : Array(indents).join('  ')
    }

    getSymbol (test) {
        const { symbols } = this.baseReporter
        let symbol = '?' // in case of an unknown state

        switch (test.state) {
        case 'pass':
            symbol = symbols.ok
            break
        case 'pending':
            symbol = '-'
            break
        case 'fail':
            symbol = symbols.err
            break
        }

        return symbol
    }

    getColor (state) {
        let color = null // in case of an unknown state

        switch (state) {
        case 'pass':
        case 'passing':
            color = 'green'
            break
        case 'pending':
            color = 'pending'
            break
        case 'fail':
        case 'failing':
            color = 'fail'
            break
        }

        return color
    }

    printSuiteResult (runner) {
        const cid = runner.cid
        const { color, stats } = this.baseReporter
        const results = stats.runners[cid]
        const preface = `[${results.sanitizedCapabilities} #${cid}]`
        const specHash = stats.getSpecHash(runner)
        let output = ''

        output += '------------------------------------------------------------------\n'
        output += `${preface} Session ID: ${results.sessionID}\n`
        output += `${preface} Spec: ${this.specs[cid]}\n`
        output += `${preface}\n`

        for (const specTitle in results.specs[specHash].suites) {
            const spec = results.specs[specHash].suites[specTitle]
            const indent = this.indent(cid, specTitle)
            output += `${preface}   ${indent}${specTitle}\n`

            for (const testTitle in spec.tests) {
                const test = spec.tests[testTitle]

                if (test.state === '') {
                    continue
                }

                output += preface
                output += '     ' + indent
                output += color(this.getColor(test.state), this.getSymbol(test))
                output += ' ' + testTitle + '\n'
            }
        }

        output += `${preface}\n`

        let displayedDuration = false
        for (const state in this.results[cid]) {
            const testCount = this.results[cid][state]
            let duration = ''

            /**
             * don't display 0 passing/pending of failing test label
             */
            if (testCount === 0) {
                continue
            }

            /**
             * set duration
             */
            if (!displayedDuration) {
                duration = ' (' + this.shortEnglishHumanizer(results.specs[specHash]._duration, this.durationOpts) + ')'
            }

            output += preface + ' '
            output += color(this.getColor(state), testCount)
            output += ' ' + color(this.getColor(state), state)
            output += duration
            output += '\n'
            displayedDuration = true
        }

        output += '\n'

        console.log(output)
    }
}

export default SpecReporter
