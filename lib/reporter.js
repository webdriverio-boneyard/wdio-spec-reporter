...
class SpecReporter extends events.EventEmitter {
    ...
    getResultList (cid, suites, preface = '') {
        ...
        for (const specTitle in suites) {
            ...
            for (const testTitle in spec.tests) {
                ...
                output += preface
                output += '       ' + indent
                output += this.baseReporter.color(this.getColor(test.state), this.getSymbol(test))
                output += ' ' + testTitle + '\n'
            }
    ...
    getSummary (states, duration, preface = '') {
        ...
        for (const state in states) {
            ...
            if (!displayedDuration) {
                duration = ' (' + this.shortEnglishHumanizer(duration, DURATION_OPTIONS) + ')'
            }
    ...
    getFailureList (cid, stats, preface) {
        let output = ''
    ...
    printSuiteResult (runner) {
        ...
        output += `${preface}\n`
        output += this.getResultList(cid, results.specs[specHash].suites, preface)
        output += `${preface}\n`
        output += this.getSummary(this.results[cid], results.specs[specHash]._duration, preface)
        output += this.getFailureList(cid, stats, preface)
