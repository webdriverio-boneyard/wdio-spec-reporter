...
class SpecReporter extends events.EventEmitter {
    ...
    getResultList (cid, suites, preface = '') {
        ...
        for (const specUid in suites) {
            ...
            for (const testUid in spec.tests) {
                ...
                output += preface
                output += '   ' + indent
                output += chalk[this.getColor(test.state)](this.getSymbol(test.state))
                output += ' ' + testTitle + '\n'
            }
    ...
    getFailureList (failures, preface) {
        ...
        failures.forEach((test, i) => {
            ...
            output += `${preface} ${chalk.red(test.err.message)}\n`
            if (test.err.stack) {
                const stack = test.err.stack.split(/\n/g).map((l) => `${preface} ${chalk.gray(l)}`).join('\n')
                output += `${stack}\n`
            } else {
                output += `${preface} ${chalk.gray('no stack available')}\n`
            }
        })
